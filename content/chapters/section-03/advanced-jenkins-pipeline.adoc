...
* Directive `parameters` : l'utilisateur fournit des arguments
au pipeline
...
* Certaines options comme `timeout` peuvent être appliquées
dans un bloc `step`

== Exercice : Options & Timeout Approval

* On souhaite que la "stage" *Approval* attende 3 minutes
...
// * Problèmes :
// ** Comment réutiliser les mêmes morceaux de Pipeline ?
// ** Comment mettre à jour toute une série de projets ?

// * Solution : Librairie de bouts de Pipeline
// ** Stockée dans un SCM (versions, branches, etc.)
// ** Combinable
// ** 
